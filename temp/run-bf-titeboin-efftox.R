...
next.dose <- function(target, d, y, y.obs, n, n.pend, elimi, lambda.e, lambda.d) {
  ...
  if (conflict) {
    ...
    if ((y_pool / n_pool) <= lambda.e & (yobs_pool / n_pool) <= target) {
      ...
      if (d != 1) {
        # next_d <- d - 1
        y_pool_tmp <- sapply(conflict_d:(d - 1), function(r) sum(y[conflict_d:r]))
        yobs_pool_tmp <- sapply(conflict_d:(d - 1), function(r) sum(y.obs[conflict_d:r]))
        n_pool_tmp <- sapply(conflict_d:(d - 1), function(r) sum(n[conflict_d:r]))
        next_d_idx <- ifelse(any((y_pool_tmp / n_pool_tmp) <= lambda.d), max(which((y_pool_tmp / n_pool_tmp) <= lambda.d)), NA)
        next_d <- ifelse(is.na(next_d_idx), max(1, conflict_d - 1), c(conflict_d:(d - 1))[next_d_idx])
      } else {
        next_d <- d
        if (n.pend > 0) suspend <- TRUE
      }
...
run.bfboin <- function(target, 
  ...
  for (trial in 1:ntrial) {
    ...
    if (earlystop == 1) {
      ...
      # top 2: recompute utility from complete follow-up data (not interim imputed uhat)
      if (length(Aset_ext) > 2) {
        if (utility.method == "mean") {
          for (dd in Aset_ext) {
            uhat[dd] <- Yeff_s1[trial, dd] / n[dd] - w0 * (Ytox_s1[trial, dd] / n[dd])
          }
        } else if (utility.method == "pp") {
          for (dd in Aset_ext) {
            qx <- (utility_score[3] * Yeff_s1[trial, dd] + utility_score[2] * (n[dd] - Ytox_s1[trial, dd])) / 100
            uhat[dd] <- 1 - pbeta(ub / 100, qx + 1, n[dd] - qx + 1)
          }
        }
...
MAIN.func <- function(target = 0.25, pT.true, pE.true, 
  ...
  ndose <- length(pT.true)
  true.mtd <- which.min(abs(pT.true - target))
  ...
  ### our method (utility posterior prob, exclude rule 2)
  result_ours1 <- run.bfboin(
    ...
    titration = FALSE, pE.low = 0.20, cutoff.eff = 0.90,
    min.complete = c(tox = 0.51, eff = eff.complete), min.MF = c(tox = 0, eff = 0),
    ntrial = nsimu, seed = seed
  )
  ...
  ### our method (utility posterior prob, no ext backfilling)
  result_ours3 <- run.bfboin(
    target = target, pT.true = pT.true, pE.true = pE.true, w0 = w0,
    ncohort = 10, cohortsize = 3, fixed.cohort = TRUE,
    accuralrate = accuralrate, DLTwindow = DLTwindow, effwindow = effwindow,
    n.earlystop = 12, n.backfilling = 12, bf.type = "utility", bf.extended = FALSE,
    utility.method = "pp", startdose = 1, init.size = 3, 
    titration = FALSE, pE.low = 0.20, cutoff.eff = 0.90,
    min.complete = c(tox = 0.51, eff = eff.complete), min.MF = c(tox = 0.25, eff = 0),
    ntrial = nsimu, seed = seed
  ...
  result_tite12 <- get.tite.boin12.approx(
    pT.true = pT.true, pE.true = pE.true, rho = 0, targetT = 0.35, targetE = 0.25, 
  ...
  selection <- rbind(
    ...
    MTD.titeet = c(result_titeet$mtd.select, result_titeet$prop.stop),
    OBD.titeet = c(result_titeet$prop.select, NA),
    MTD.tite12 = c(result_tite12$sel_mtd, result_tite12$earlystop), 
    OBD.tite12 = c(result_tite12$sel, NA), 
    MTD.bfet = c(rep(NA, ndose), result_bfet$prop.stop), 
    OBD.bfet = c(result_bfet$prop.select, NA)
  ) %>% round(2)
  ...
  EN <- rbind(
    ...
    titeet = c(result_titeet$n.patient, sum(result_titeet$n.patient), EN_overdose$titeet, sum(result_titeet$n.tox)),
    tite12 = c(result_tite12$pts, sum(result_tite12$pts), EN_overdose$tite12, sum(result_tite12$dlt)),
...
### settings ========================
pT.true <- rbind(
  c(0.15, 0.25, 0.35, 0.40, 0.45),
  c(0.05, 0.15, 0.25, 0.35, 0.40),
  c(0.02, 0.06, 0.10, 0.25, 0.35),
  c(0.01, 0.03, 0.06, 0.10, 0.25),
  
  c(0.05, 0.12, 0.22, 0.35, 0.40),
  c(0.02, 0.06, 0.10, 0.22, 0.35),
  
  c(0.05, 0.15, 0.25, 0.35, 0.40),
  c(0.08, 0.12, 0.18, 0.25, 0.35),
  c(0.02, 0.06, 0.18, 0.25, 0.35),
  c(0.05, 0.08, 0.15, 0.18, 0.25)
)
pE.true <- rbind(
  c(0.35, 0.35, 0.36, 0.37, 0.37),
  c(0.15, 0.35, 0.36, 0.37, 0.37),
  c(0.05, 0.10, 0.35, 0.36, 0.37),
  c(0.05, 0.10, 0.15, 0.35, 0.36),
  
  c(0.10, 0.15, 0.38, 0.38, 0.40),
  c(0.05, 0.10, 0.15, 0.38, 0.40),
  
  c(0.35, 0.35, 0.36, 0.37, 0.37),
  c(0.38, 0.38, 0.39, 0.40, 0.40),
  c(0.15, 0.35, 0.36, 0.37, 0.37),
  c(0.15, 0.38, 0.38, 0.39, 0.40)
)
...
all_config <- expand.grid(
  ...
  eff_complete =  0.31 #c(0.21, 0.31, 0.41)
)
...
output$selection <- cbind(setting.idx = sc00, output$selection) %>% 
  as.data.frame() %>% tibble() %>% 
  mutate(method = c("MTD.ours0", "OBD.ours0", "MTD.bf", "OBD.bf", 
                    "MTD.ours1", "OBD.ours1", "MTD.ours2", "OBD.ours2", 
                    "MTD.ours3", "OBD.ours3", "MTD.titeet", "OBD.titeet", 
                    "MTD.tite12", "OBD.tite12", "MTD.bfet", "OBD.bfet"), .after = 1)
output$EN <- cbind(setting.idx = sc00, output$EN, duration = output$duration) %>% 
  as.data.frame() %>% tibble() %>% 
  mutate(method = c("ours0.s1", "ours0.s2", "ours0.total", "bf.s1", "bf.s2", "bf.total", 
                    "ours1.s1", "ours1.s2", "ours1.total", "ours2.s1", "ours2.s2", "ours2.total",  
                    "ours3.s1", "ours3.s2", "ours3.total", "titeet", "tite12", "bfet"), .after = 1)
