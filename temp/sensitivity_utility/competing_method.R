### TITE-BOIN-ET ============
my_tite.boinet <- function(n.dose, start.dose, size.cohort, n.cohort, toxprob,
                           effprob, phi = 0.3, phi1 = phi * 0.1, phi2 = phi * 1.4, delta = 0.6,
                           delta1 = delta * 0.6, alpha.T1 = 0.5, alpha.E1 = 0.5, tau.T,
                           tau.E, te.corr = 0.2, gen.event.time = "weibull", accrual,
                           gen.enroll.time = "uniform", stopping.npts = size.cohort *
                             n.cohort, stopping.prob.T = 0.95, stopping.prob.E = 0.99,
                           estpt.method = "obs.prob", obd.method = "max.effprob", w1 = 0.33,
                           w2 = 1.09, plow.ast = phi1, pupp.ast = phi2, qlow.ast = delta1 / 2,
                           qupp.ast = delta, psi00 = 40, psi11 = 60, n.sim = 1000, seed.sim = 100) {
  if (length(toxprob) != n.dose) {
    ...
    if (gen.event.time == "weibull") {
      for (i in 1:n.dose) {
        ...
        mv.ncop <- append(mv.ncop, copula::mvdc(
          copula = ncop,
          margins = c("weibull", "weibull"), paramMargins = list(list(shape = zetta.T1, scale = zetta.T2), list(shape = zetta.E1, scale = zetta.E2))
        ))
      }
      ...
      for (i in 1:n.dose) {
        ...
        mv.ncop <- append(mv.ncop, copula::mvdc(
          copula = ncop,
          margins = c("unif", "unif"), paramMargins = list(list(
            min = 0,
            max = tau.T * (1 / psi.T)
    ...
    data.obs.tox <- data.obs.n <- array(0, dim = c(n.sim, n.dose))
    d_mtd <- data.dur <- obd <- array(0, dim = c(n.sim))
    set.seed(seed.sim)
    for (ss in 1:n.sim) {
      ...
      for (i in 1:nesc) {
        ...
        tite.df <- rbind(
          tite.df,
          data.frame(dose = curdose, enter = t.enter, endtox = t.enter + apply(as.matrix(1:ncoh), 1, function(x) {
            min(time.te[x, 1], tau.T)
          }), dlt = DLT, endeff = t.enter + apply(as.matrix(1:ncoh), 1, function(x) {
        ...
        while (mean(gamma.T * gamma.E) < 0.5) {
          ...
          ### ?? refine enrolling time
          if (gen.enroll.time == "uniform") {
            t.decision <- min(nextdec.T, nextdec.E) + runif(1, 0, 2 * accrual)
          } else if (gen.enroll.time == "exponential") {
            t.decision <- min(nextdec.T, nextdec.E) + rexp(1, 1 / accrual)
          }
          gamma.T <- as.numeric(tite.curdose$endtox <= t.decision)
          gamma.E <- as.numeric(tite.curdose$endeff <= t.decision)
        }
      ...
      data.obs.tox[ss, ] <- obs.tox
      data.obs.n[ss, ] <- obs.n
      data.dur[ss] <- t.decision
      evadose <- dosen[obs.n != 0]
      d_mtd[ss] <- select.mtd(
        target = phi, npts = obs.n, ntox = obs.tox, cutoff.eli = 0.95,
        extrasafe = FALSE, offset = 0.05, boundMTD = FALSE, p.tox = phi2
      )$MTD
      ...
      if (early.stop == 1) {
        ...
        (eterm.obd[evadose] >= (1 - stopping.prob.E))) >= 1) {
        ...
        if (estpt.method == "multi.iso") {
          ...
          estpe <- fp.logit(
            obs = obs.eff[evadose], n = obs.n[evadose],
            dose = evadose
          )
    ...
    mtd.select <- prop.select <- array(0, dim = c(n.dose))
    for (i in 1:n.dose) {
      mtd.select[i] <- round(mean(d_mtd == i) * 100, digits = 1)
      prop.select[i] <- round(mean(obd == i) * 100, digits = 1)
    }
    names(prop.select) <- names(mtd.select) <- dose
    prop.stop <- round(mean(obd == 0) * 100, digits = 1)
    names(prop.stop) <- "No OBD %"
    n.tox <- round(apply(data.obs.tox, 2, mean), digits = 1)
    ...
    result <- list(
      ...
      n.tox = n.tox, n.patient = n.patient,
      mtd.select = mtd.select, prop.select = prop.select,
...
get.boundary <- function(
    target, targetE, ncohort, cohortsize = 3, p.saf = NA, p.tox = NA, 
    cutoff.eli = 0.95, cutoff.eli.E = 0.90) {
...
get.tite.boin12.approx <- function(
    pT.true, pE.true, rho, targetT = 0.35, targetE = 0.25, ncohort = 10, 
    cohortsize = 3, startdose = 1, accrual.rate = 2, 
    arrive.dist = "Uniform", event.dist = "Weibull", A = c(2, 3), 
    maxpen = 0.5, n.earlystop = ncohort * cohortsize, 
    p.saf = 0.6 * targetT, p.tox = 1.4 * targetT, cutoff.eli.T = 0.95, 
    N1 = 6, N2 = 9, cutoff.eli.E = 0.90, u11 = 60, u00 = 40, 
    ntrial = 100, rseed = 200, alpha = c(0.5, 0.5), RunIn = FALSE) {
  set.seed(rseed)
